...
async function createAgentVersion(agentId: number, overrides?: Partial<CreateAgentDTO>) {
  const previousAgent = await getAgent(agentId);
  if (!previousAgent) {
    throw new Error('Agent not found');
  }

  const { agent_id, created_at, updated_at, ...agentData } = previousAgent;
...
async function listAgents(paginationQuery?: Partial<PaginationQuery>) {
  ...
  // Get total count of unique agents (by name)
